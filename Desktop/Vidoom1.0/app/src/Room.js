...
module.exports = class Room {
    constructor(room_id, worker, io, userPlan) {
        this.id = room_id;
        this.worker = worker;
        ...
        this.maxPeers = this.planLimits[userPlan] || 3;
        // ##########################
    ...
    toJson() {
        return {
            ...
            peers: this.peers.size > 0 ? JSON.stringify([...this.peers]) : '[]',
        };
    }
    ...
    // Modify the addPeer method to include the check
    addPeer(peer) {
        if (this.canAddPeer()) {
            this.peers.set(peer.id, peer);
        } else {
            throw new Error('Room capacity full for the current plan');
        }
    }
    ...
    async createWebRtcTransport(socket_id) {
        ...
        if (maxIncomingBitrate) {
            try {
                ...
                log.error('Error setting maxIncomingBitrate', error.message);
            }
        }
  ...
  async consume(socket_id, consumer_transport_id, producer_id, rtpCapabilities) {
    try {
        // Tekshirish: socket_id uchun peer mavjudligini aniqlang
        if (!this.peers.has(socket_id)) {
            console.warn(`Peer with socket ID ${socket_id} not found`);
            return;
        }

        // Tekshirish: iste'mol qilish imkoniyatlari
        if (
        ...
        // Peer obyektini oling
        const peer = this.getPeer(socket_id);
        if (!peer) {
            console.error(`Peer object not found for socket ID ${socket_id}`);
            return;
        }

        // Consumer yarating
        const peerConsumer = await peer.createConsumer(
            consumer_transport_id,
            producer_id,
            rtpCapabilities
        );
        ...
        if (!peerConsumer) {
            ...
            throw new Error(
                `Peer consumer not found for transport ID ${consumer_transport_id} and producer ID ${producer_id}`
            );
        }

        // Consumer va uning parametrlarini ajratib oling
        const { consumer, params } = peerConsumer;
        const { id, kind } = consumer;

        // Consumer yopilganda event ishlovchisi
        consumer.on('producerclose', () => {
            console.debug('Consumer closed due to "producerclose" event', {
                consumer_id: id,
                consumer_kind: kind,
            });

            // Peer obyektidan consumerni olib tashlang
            peer.removeConsumer(id);

            // Klientga xabar yuboring
        ...
        console.info('Consumer successfully created', {
            consumer_id: id,
            consumer_kind: kind,
            params,
        });

        // Iste'molchining parametrlari qaytariladi
        return params;
    } catch (error) {
        console.error('Error in consume function:', {
            error: error.message,
            socket_id,
            consumer_transport_id,
            producer_id,
        ...
        throw error; // Xatolikni yuqoriga o'tkazish
    }
}
