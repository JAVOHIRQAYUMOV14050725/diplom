...
const html = {
    ...
    userName: 'username',
    userHand: 'fas fa-hand-paper pulsa  te',
    pip: 'fas fa-images',
...
// Xonani yaratgan foydalanuvchi ismi va maksimal ishtirokchilar soni
let roomOwner = window.localStorage.getItem('room_owner');
let storedMaxParticipants = window.localStorage.getItem('max_participants');
// Hozirgi foydalanuvchi ismi (logindan olingan)
let user_name = window.sessionStorage.getItem('user_name');
let userPlan = localStorage.getItem('plan');
let secretKey = 'vidoom1.0_secret_key'
let plan

class RoomClient {
    constructor(localAudioEl, remoteAudioEl, videoMediaContainer, videoPinMediaContainer, mediasoupClient, socket, room_id, peer_name, peer_uuid, peer_info, isAudioAllowed, isVideoAllowed, isScreenAllowed, joinRoomWithScreen, isSpeechSynthesisSupported, transcription, successCallback,) {
        ...
        this.createRoom(this.room_id, userPlan).then(async () => {
            const data = {
        ...
        this.socket
            ...
            .then(async (room) => {
                ...
                if (room === 'fullCapacity') {
                    console.log('00-WARNING ----> Room is full. Cannot join.');
                    return this.roomIsFull();  // Agar qatnashuvchilar soni maksimalga yetsa kirishni to'xtatadi
                }
                if (room === 'invalid') {
                ...
                if (room === 'notAllowed') {
                    console.log('00-WARNING ----> Room is Unauthorized for current user, please provide a valid room name for this user');
                    return this.userRoomNotAllowed();
                }
                if (room === 'unauthorized') {
                    console.log('00-WARNING ----> Room is Unauthorized for current user, please provide a valid username and password');
                ...
                const peers = room.peers && room.peers !== '[]' ? new Map(JSON.parse(room.peers)) : new Map();  // Agar room.peers mavjud va bo'sh bo'lmasa, parse qilamiz

                // Agar xonani yaratgan foydalanuvchi hali saqlanmagan bo'lsa, ya'ni, birinchi foydalanuvchi xonani yaratgan bo'lsa
                if (!user_name) {
                    // Agar foydalanuvchi ismi localStorage-da saqlanmagan bo'lsa, uni sessionStorage'dan o'qiymiz
                    user_name = window.sessionStorage.getItem('user_name');
                    // Foydalanuvchi ismini localStorage'ga saqlaymiz, shunda sahifa yangilansa ham saqlanib qoladi
                    window.localStorage.setItem('user_name', user_name);

                }
                window.localStorage.setItem('peer_name', peer_name)

                // Agar foydalanuvchi tokeni yo'q bo'lsa, foydalanuvchi ismiga ko'ra tekshirish
                if (!data.peer_token) {
                    // `peer_id` va `peer_name` ni to'g'ri aniqlash
                    const peer_id = this.peer_id;  // `this` ichida peer_id aniqlangan bo'lishi kerak
                    const peer_name = this.peer_name;  // `this` ichida peer_name aniqlangan bo'lishi kerak

                    for (let peer of Array.from(peers.keys()).filter((id) => id !== peer_id)) {
                        let peer_info = peers.get(peer).peer_info;
                        if (peer_info.peer_name === peer_name) {
                ...
                // Xonaga qo'shilishga ruxsat berish
                return this.joinAllowed(room);
            })
            .catch((error) => {
    ...
    async initTransports(device) {
        ...
        const producerTransportData = await this.socket.request('createWebRtcTransport', {
            forceTcp: false,
            iceServers: [
                { urls: 'stun:213.230.91.183:3478' },  // STUN server
                {
                    urls: 'turn:213.230.91.183:3478',  // TURN server
                    username: 'testuser',              // TURN server foydalanuvchi
                    credential: 'securepassword123',   // TURN server parol
                }
            ],
        ...
        this.producerTransport.on('connect', async ({ dtlsParameters }, callback, errback) => {
            try {
                console.log('Connecting to TURN/STUN server with DTLS parameters:', dtlsParameters);
                await this.socket.request('connectTransport', {
                    transport_id: this.producerTransport.id,
                    dtlsParameters,
                });
                callback();
                console.log('Successfully connected to the TURN/STUN server.');
            } catch (err) {
                errback(err);
                console.error('Failed to connect to TURN/STUN server:', err);
            }
        });
 ...
 this.producerTransport.on('connectionstatechange', async (state) => {
    try {
        switch (state) {
            ...
            case 'disconnected':
                ...
                if (this.producerTransport.connectionState !== 'closed') {
                    console.warn('Restarting ICE due to disconnection...');
                    await this.restartIce();
                } else {
                    console.error('Transport is closed, cannot restart ICE');
                }
            ...
            case 'failed':
                ...
                if (this.producerTransport.connectionState !== 'closed') {
                    console.warn('Attempting to restart ICE before closing transport...');
                    await this.restartIce();
                }

                // Agar qayta ishga tushirish muammoni hal qilmasa, transportni yopamiz
                this.producerTransport.close();
                console.error('Transport failed, closed the transport.');
        ...
        console.error('Error during connectionstatechange handler:', error);
    }
});
        ...
        this.producerTransport.on('icegatheringstatechange', (state) => {
            console.log('Producer ICE gathering state changed:', state);
            if (state === 'complete') {
                console.log('ICE candidates gathered:', this.producerTransport.iceCandidates);
            }
        });
    ...
    initSockets() {
        ...
        this.socket.on('newProducers', this.handleNewProducers); // Yangi prodyuserlar uchun hodisa
    ...
    roomIsFull() {
        this.msgPopup("warning", 'Комната заполнена. Не могу присоединиться.');
    }

    // Bu metod ishtirokchilar sonini qaytaradi
    getRoomParticipantsCount() {
        return this.participantsCount;
    }

    // Qachon ishtirokchi qo'shilsa yoki ketsa, ishtirokchilar sonini yangilash
    updateParticipantsCount(newCount) {
        this.participantsCount = newCount;
    }
    ...
    async produce(type, deviceId = null, swapCamera = false, init = false) {
        ...
        try {
            ...
            // Specific error handling for media device issues
            if (err.name === 'NotAllowedError') {
                console.error('User denied media permissions');
                this.userLog('error', 'Permission denied for accessing media devices.', 'top-end');
            } else if (err.name === 'NotFoundError') {
                console.error('No media devices found');
                this.userLog('error', 'No available media devices found. Please check your device.', 'top-end');
            }
        
            // Handle video quality issues
            if (!audio && !screen && videoQuality.selectedIndex != 0) {
                videoQuality.selectedIndex = this.videoQualitySelectedIndex;
                this.sound('alert');
                this.userLog(
                    'error',
                    `Your device doesn't support the selected video quality (${videoQuality.value}), please select another one.`,
                    'top-end',
                );
            }
        }
    ...
    async setupPeerConnection(sdp) {
        // TURN/STUN serverlarni qo‘lda belgilash
        const iceServers = [
            { urls: 'stun:213.230.91.183:3478' }, // STUN server
            {
                urls: 'turn:213.230.91.183:3478', // TURN server
                username: 'testuser',             // TURN server foydalanuvchi nomi
                credential: 'securepassword123',  // TURN server paroli
            },
        ];
        ...
        // Video va audio oqimlarini boshqarish
        ...
        // Data channel hodisalarini boshqarish
        this.peerConnection.ondatachannel = (event) => {
            event.channel.onmessage = this.handleVideoAIMessage;
        };

        // SDP (Session Description Protocol) ni o‘rnatish
        const remoteDescription = new RTCSessionDescription(sdp);
        await this.peerConnection.setRemoteDescription(remoteDescription);

        console.log('PeerConnection successfully setup with static ICE servers.');
    }
