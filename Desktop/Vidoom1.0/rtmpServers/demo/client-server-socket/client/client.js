...
async function startCapture(constraints) {
    try {
        ...
        attachMediaStream(stream);  // Attach the media stream to the appropriate element
        return stream;
    } catch (err) {
        console.error('Error accessing media devices.', err);
        if (err.name === 'NotAllowedError') {
            showError('Permission denied for accessing media devices. Please allow access to the camera and microphone.');
        } else if (err.name === 'NotFoundError') {
            showError('No available media devices found. Please check your camera and microphone settings.');
        } else {
            showError('An unknown error occurred while accessing media devices.');
        }
    }
}
